...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/03 21:18:35 by ryaoi             #+#    #+#             */
/*   Updated: 2016/12/08 18:00:34 by ryaoi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void				ft_print_capx_zero(t_pf **pf)
{
	int					i;
...
static void				ft_print_capx_space(t_pf **pf)
{
	char				*tmp;
	size_t				size;
	int					i;
...
static void				ft_printf_capx_precision(t_pf **pf,
...
static void				pre_digit(t_pf **pf, int *flag_p,
						unsigned long long nbr)
{
	ft_printf_capx_precision(pf, nbr);
	(*flag_p) = 1;
}
...
void					ft_printf_capx(t_pf **pf, va_list ap)
{
	...
	if (((*pf)->width > (*pf)->precision && (*pf)->width >
		...
		ft_print_capx_space(pf);
	if (((*pf)->width > (*pf)->precision || (*pf)->width >
		ft_ucount_digit_base(nbr, 16)) && ((*pf)->flag_zero)
		&& flag_p != 1 && (*pf)->flag_minus == 0)
		ft_print_capx_zero(pf);
	if ((*pf)->flag_minus == 1)
		ft_stradjust(pf);
	if ((*pf)->flag_diese == 1)
		add_prefix_hex(pf);
	ft_check_hex(pf);
	ft_str_capitalize(&(*pf)->result);
	ft_putstr((*pf)->result);
