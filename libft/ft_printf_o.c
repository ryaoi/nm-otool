...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/08 18:26:28 by ryaoi             #+#    #+#             */
/*   Updated: 2016/12/08 18:49:54 by ryaoi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void				ft_print_o_zero(t_pf **pf)
{
	int					i;
...
static void				ft_print_o_space(t_pf **pf)
{
	char				*tmp;
	size_t				size;
	int					i;
...
static void				ft_printf_o_precision(t_pf **pf, unsigned long long nbr)
{
	char				*str;
	int					i;
	int					j;
	...
	i = (*pf)->precision - ft_ucount_digit_base(nbr, 8);
...
void					ft_printf_o(t_pf **pf, va_list ap)
{
	...
	if ((*pf)->precision > ft_ucount_digit_base(nbr, 8))
		pre_digit(pf, &flag_p, nbr);
	if (((*pf)->width > (*pf)->precision &&
		(*pf)->width > ft_ucount_digit_base(nbr, 8)))
		ft_print_o_space(pf);
	if (((*pf)->width > (*pf)->precision || (*pf)->width >
		ft_ucount_digit_base(nbr, 8)) && ((*pf)->flag_zero)
